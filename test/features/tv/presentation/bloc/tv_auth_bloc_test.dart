...
void main() {
  ...
  group('TvAuthBloc', () {
    ...
    test('TvAuthStarted emits [TvAuthChecking, TvAuthPaired] when token exists', () async {
    ...
    test('TvAuthStarted emits [TvAuthChecking, TvPairingLoading, TvPairingPending] when no token', () async {
    ...
    test('TvPairingPollTicked emits TvAuthPaired when status is approved', () async {
      repository.createSessionResult = success(TvPairingSession(
        sessionId: 'sess-1',
        userCode: 'AB7K9Q',
        verificationUrl: 'https://app.akademihub.id/tv-pair',
        expiresAt: DateTime.now().toUtc().add(const Duration(minutes: 10)),
      ...
      repository.pairingStatusResult = success(const TvPairingStatus(
    ...
    test('TvPairingPollTicked emits TvPairingDenied when status is denied', () async {
      bloc.add(const TvPairingRequested());
      await expectLater(
        bloc.stream,
        emitsInOrder([
      ...
      repository.pairingStatusResult = success(const TvPairingStatus(
        state: TvPairingState.denied,
      ));

      bloc.add(const TvPairingPollTicked());
      await expectLater(
        bloc.stream,
        emits(isA<TvPairingDenied>()),
      );
    });

    test('TvPairingPollTicked emits TvPairingExpired when session is expired', () async {
      repository.createSessionResult = success(TvPairingSession(
        sessionId: 'sess-1',
        userCode: 'AB7K9Q',
        verificationUrl: 'https://app.akademihub.id/tv-pair',
      ...
      await expectLater(
        bloc.stream,
        emits(isA<TvPairingExpired>()),
      );
    });

    test('TvUnpairRequested calls unpair and requests new pairing session', () async {
